...
object Day15Part2 {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val map_filling_pr = new ProgramRunner(createProgram(file_lines))
        val path_finding_pr = new ProgramRunner(createProgram(file_lines))

        fillMap(map_filling_pr)

        val pos = (MAP_SIZE / 2, MAP_SIZE / 2)
        val path = bfs(pos, true)
        val os_pos = move(pos, path_finding_pr, path)

        val depth = floodFill(os_pos)
        println(depth - 1)
    }
    ...
    def floodFill(pos: (Int, Int)): Int = {
        if (pos._1 < 0 || pos._2 < 0 || pos._1 >= MAP_SIZE || pos._2 >= MAP_SIZE || !isTraversable(pos)) 0
        else {
            putMap(pos, OXYGENATED)
            1 + getAdjacent(pos).map(floodFill).max
        }
    }
}
