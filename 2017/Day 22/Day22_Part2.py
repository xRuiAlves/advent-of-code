...
def is_weakened(node_id):
    return (node_id == weakenedNode)

def is_flagged(node_id):
    return (node_id == flaggedNode)
...
def main():
    ...
    for i in range(num_bursts):
        if (is_clean(matrix[y][x])):
            current_dir = turn_left(current_dir)
            matrix[y][x] = weakenedNode
        elif (is_weakened(matrix[y][x])):
            matrix[y][x] = infectedNode
            infection_counter += 1
        elif (is_infected(matrix[y][x])):
            current_dir = turn_right(current_dir)
            matrix[y][x] = flaggedNode
        else:
